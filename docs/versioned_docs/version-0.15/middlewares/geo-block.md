...
Save it as **`/etc/goma/country.mmdb`** and Goma picks it up with no configuration. To keep it elsewhere, name the path in `goma.yml`:
...
or set `GOMA_GEOIP_DB`, which overrides the file:

```bash
GOMA_GEOIP_DB=/srv/geoip/dbip-country-lite.mmdb
```

An explicitly configured path is used exactly as given — Goma will not quietly fall back to another file, because geo rules deciding on a database nobody chose is worse than geo rules that do not run.
